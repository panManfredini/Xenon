...
AmBeTest::AmBeTest(TTree *T2,TTree *T3, TTree *T1) : XenonDataReader(T2,T3,T1) 
	{
	}
...
//	if((*S1s)[0].empty() || (*S2s)[0].empty()) continue;
  ...
  TH1F *AmBeTest = new TH1F("AmBeTest","AmBeTest VS dataset time",80000,1200000000.,1500000000.); // unixtime from 208 to 2017 in bin o ~1h
  AmBeTest->GetXaxis()->SetTitle("Date");
  AmBeTest->GetYaxis()->SetTitle("Event Rate / day [day^{-1}]");
  AmBeTest->GetXaxis()->SetTimeDisplay(1);
  AmBeTest->GetXaxis()->SetTimeFormat("%b");
  
  AmBeTest->Fill( XeT1.TimeSec, (float)nuber_of_survived_event/livetime);
  AmBeTest->SetBinError(AmBeTest->FindBin( XeT1.TimeSec), sqrt((float)nuber_of_survived_event)/livetime);
